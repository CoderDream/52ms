...
public class ControlServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action.equals("toregister")) {// 到注册
			...
			int temNum = UserInfoDao.insertUser(u_name, u_qq, u_pwd, u_Email, u_dis);
			if (temNum != -1) {// 插入数据成功
			...
			String uid = UserInfoDao.checkUser(u_name, u_pwd);
			if (uid != null) {
				HttpSession session = request.getSession(true);
			...
			String uid = (String) session.getAttribute("uid");
			if (uid != null) {
				UserInfoDao.updateUserHead(Integer.parseInt(uid), Integer.parseInt(id));
				request.getRequestDispatcher("home.jsp").forward(request, response);
			} else {
				request.getRequestDispatcher("error.jsp?msg=请登录").forward(request, response);
			}
			...
			UserInfoDao.updateUserInfo(uid, u_name, u_qq, u_Email, u_favourite, u_dis);
			request.getRequestDispatcher("home.jsp").forward(request, response);
		} else if (action.equals("admin")) {// 进入管理界面
			request.getRequestDispatcher("admin.jsp").forward(request, response);
		} else if (action.equals("addToRecommend")) {// 添加推荐动作
			String mid = request.getParameter("mid");
			MstxDao.insertMstxRecommend(Integer.parseInt(mid));
			request.getRequestDispatcher("admin.jsp").forward(request, response);
		} else if (action.equals("deleteMstx_recommend")) {// 删除每日推荐
			String mid = request.getParameter("mid");
			MstxDao.deleteMstxRecommend(Integer.parseInt(mid));
			request.getRequestDispatcher("everydayRecommend.jsp").forward(request, response);
