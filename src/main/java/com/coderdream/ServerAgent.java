...
import com.coderdream.dao.MstxDao;
import com.coderdream.dao.MstxImageDao;
import com.coderdream.dao.MstxInfoDao;
import com.coderdream.dao.UserInfoDao;
		...
		while (flag) {// 循环
			try {
				...
				if (msg.startsWith("<#LOGIN#>")) {// 登录动作
					...
					String u_name = UserInfoDao.checkUser(str[0], str[1]);// 检查数据库中是否含有该用户
					...
					String[] str = msg.split("\\|");// 分割字符串
					int uid = UserInfoDao.insertUser(str[0], str[2], str[1], str[3], str[4]);
					...
					int totleNumber = MstxInfoDao.getMstxInfoCountForPhone(str[0], Integer.parseInt(str[1]), str[2], str[3]);

					List<String[]> mstxInfos = MstxInfoDao.getMstxInfoForPhone(str[0], Integer.parseInt(str[1]), str[2], str[3],
					...
					for (String[] mi : mstxInfos) {// 循环组织字符串发送到客户端
						dout.writeUTF(mi[0] + "|" + mi[1] + "|" + mi[2] + "|" + mi[3] + "|" + mi[4] + "|" + mi[5] + "|" + mi[6]
								+ "|" + mi[7]);
					}
					...
					ArrayList<Blob> blobs = MstxImageDao.getMstxImage(mids);
					for (Blob b : blobs) {
					...
					ArrayList<MstxInfo> mstxInfos = MstxInfoDao.getFavourite(msg);
					dout.writeUTF("<#FAVOURITEINFO#>" + mstxInfos.size());// 向客户端发送搜索的数据
					for (MstxInfo mi : mstxInfos) {// 循环组织字符串发送到客户端
						dout.writeUTF(mi.getInfo_title() + "|" + mi.getInfo_dis() + "|" + mi.getInfo_lon() + "|" + mi.getInfo_lat()
								+ "|" + mi.getInfo_time() + "|" + mi.getUid() + "|" + mi.getMid());
					}
					int[] mids = new int[mstxInfos.size()];// 创建存放MID数组
					for (int i = 0; i < mids.length; i++) {
					...
					ArrayList<Blob> blobs = MstxImageDao.getMstxImage(mids);// 得到需要的图片列表
					for (Blob b : blobs) {
						int size = (int) b.length();
						byte[] bs = b.getBytes(1, (int) size);
						dout.writeInt(size);// 写入字节数组的长度
					...
					ArrayList<MstxInfo> mstxInfos = MstxInfoDao.getMstxRecommend();
					...
					String[] str = msg.split("\\|");// 分割字符串
					MstxDao.deleteMstxCol(str[0], str[1]);// 删除数据库中的该收藏
				} else if (msg.startsWith("<#INSERTMSTXCOL#>")) {// 收藏动作
					msg = msg.substring(17);// 截取子串
					String[] str = msg.split("\\|");// 分割字符串
					MstxDao.insertMstxCol(str[0], str[1], "");// 第三个参数为评论，此处并没有实现
				} else if (msg.startsWith("<#INSERTMSTXINFO#>")) {
					msg = msg.substring(18);// 截取子串
					String[] str = msg.split("\\|");// 分割字符串
					int mid = MstxInfoDao.insertMstxInfo(str[0], str[1], str[2], str[3], str[4], str[5],
							Integer.parseInt(str[6]), 2, str[7]);

					int size = din.readInt();// 读取图片数组的长度
					byte[] image = new byte[size];// 创建图片数组
					din.read(image);// 读取图片数组

					MstxImageDao.insertMstxImage(image, mid);// 插入到数据库
