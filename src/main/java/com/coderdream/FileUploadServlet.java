...
public class FileUploadServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (action.equals("insertMstxInfo")) {// 上传推荐动作
			String info_title = request.getParameter("info_title");// 美食名称
			...
			int mid = MstxInfoDao.insertMstxInfo(info_title, info_dis, info_lon, info_lat, uid, info_sort, info_price,
							1, info_hotel);
			...
			try {
				...
				while (i.hasNext()) {
					...
					if (fileName != null) {
						...
						MstxImageDao.insertMstxImage(fullFile, mid);// 存储到数据库
						// 存储到硬盘中的文件
			...
			try {
				...
				while (i.hasNext()) {
					...
					if (fileName != null) {
						if (!fileName.equals("")) {
							File fullFile = new File(fi.getName());
							MstxDao.insertMstxHead(tdis, fullFile, Integer.parseInt(uid));// 存储到数据库
						}
					}
				}
			...
			int id = MstxDao.updateIntoAdsInfo(Integer.parseInt(gid));
			try {
				// 实例化一个硬盘文件工厂,用来配置上传组件ServletFileUpload
				DiskFileItemFactory factory = new DiskFileItemFactory();
				factory.setSizeThreshold(4096);// 设置缓冲区大小，这里是4kb
				...
				while (i.hasNext()) {
					...
					if (fileName != null) {
						if (!fileName.equals("")) {
							File fullFile = new File(fi.getName());
							MstxDao.insertMstxAdsImage(fullFile, id);// 存储到数据库
						}
					}
				}
